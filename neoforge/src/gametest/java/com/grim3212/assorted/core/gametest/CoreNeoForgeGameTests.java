 ...
 * {@code AssortedCoreForge}: nothing in {@code main} may reference the gametest source set, or the
 * tests would have to live in {@code main} and would ship in the jar. FML discovers this class by
 * scanning the mod's own classes, and in a release build it is simply not there.
 * <p>
...
public final class CoreNeoForgeGameTests {

    private CoreNeoForgeGameTests() {
    }

    @SubscribeEvent
    public static void registerGameTests(final RegisterEvent event) {
        event.register(Registries.TEST_FUNCTION, helper -> CoreGameTests.forEach(

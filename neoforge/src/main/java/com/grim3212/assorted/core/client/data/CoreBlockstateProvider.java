...
/**
 * Forge's {@code BlockStateProvider} and its {@code ModelFile} / {@code ConfiguredModel} builders
 * are gone. Block states and models are generated by vanilla's {@link ModelProvider} now, which
 * hands a {@link BlockModelGenerators} to {@link #registerModels}; a block state is a
 * {@link MultiVariantGenerator} built from {@code PropertyDispatch}es rather than a per-state
 * lambda.
 * <p>
 * The item half of the same provider is {@link CoreItemModelProvider}. Because one
 * {@link ModelProvider} writes both halves, the two are kept apart by narrowing what each one
 * claims to know about - this provider owns every block plus the block items, which
 * {@link ModelProvider} itself models by pointing them at the block model.
 */
public class CoreBlockstateProvider extends ModelProvider {
    ...
    /**
     * The old builder walked every state and set a Y rotation from
     * {@code FACING.getOpposite().toYRot()} - north 0, east 90, south 180, west 270, which is
     * exactly {@link BlockModelGenerators#ROTATION_HORIZONTAL_FACING} - so the rotation is a shared
     * dispatch now instead of a hand-computed angle.
     */

...
public class EnergyCellTileEntity extends TileEntity implements ITickableTileEntity, IEnergyProvider {

	@Nonnull
	private CoreEnergyStorage storage = new CoreEnergyStorage(1000, 100);

	public EnergyCellTileEntity() {
		super(CoreTileEntityTypes.ENERGY_CELL.get());
	}

	@Override
	public void tick() {
		AssortedCore.LOGGER.info("energy_cell stored energy :? " + this.storage.getEnergyStored() + " / " + this.storage.getMaxEnergyStored());
	}
